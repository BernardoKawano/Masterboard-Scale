...
export const PIPELINE_STAGES = [
  ...
  { id: 'convidado_evento', label: 'Convidado evento', color: '#EAB308' },
  { id: 'assinado', label: 'Assinado / pago', color: '#22C55E' },
...
export function conversionRate(records) {
  ...
  const prospeccao = eligible.filter((record) => {
    const stage = resolvePipelineStage(record);
    return stage === 'prospeccao' || stage === 'diagnostico_enviado' || stage === 'reuniao_diagnostico' || stage === 'negociacao' || stage === 'convidado_evento' || stage === 'assinado';
  }).length;
